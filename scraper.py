...
# Illustrations
for num in range(1):
    os.mkdir(f'Pictures') if not os.path.exists(f'Pictures') else None
    site = f'https://glucosetl.wordpress.com/roshidere-v4-5-illustrations/'
    r = requests.get(f'https://glucosetl.wordpress.com/roshidere-v4-5-illustrations/')
    soup = BeautifulSoup(r.text, "html.parser")
    image_tags = soup.find_all('img')
    urls = [img['src'] for img in image_tags][1:-1]
    for url in urls:
